...
public class RankingByAttribute extends Ranking {
	...
	@Override
	protected List<RankingPosition> getUpdatedRank(List<Player> players) {
		...
		for (int i = 0; i < sorted.size(); i++) {
			...
			if (comparator.compare(current, next) != 0) {
				List<String> finalScore = attributeName.stream()
						.map(a -> (next.getAttribute(a) != null && next.getAttribute(a).getValue() != null ? next.getAttribute(a).getValue().toString() : "no")
								+ " " + a)
						.toList();
