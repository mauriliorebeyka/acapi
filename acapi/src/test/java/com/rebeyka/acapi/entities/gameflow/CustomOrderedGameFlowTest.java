...
public class CustomOrderedGameFlowTest {
	...
	public void setup() {
		...
		when(player1.getAttribute("VP")).then(_ -> new Attribute<Integer>("",10,Types.integer(), null));
		when(player2.getAttribute("VP")).then(_ -> new Attribute<Integer>("",20,Types.integer(), null));
		when(player3.getAttribute("VP")).then(_ -> new Attribute<Integer>("",5,Types.integer(), null));
