...
public class ActionableCheckTest {
	...
	public void test() {
		...
		Attribute<String> attribute = new Attribute<String>("55");
		...
		when(player.getAttribute("HP", String.class)).thenReturn(attribute);
		when(player.getId()).thenReturn("PLAYER ID");
		when(player.getAttribute("int", Integer.class)).thenReturn((Attribute<Integer>)intAttribute);
